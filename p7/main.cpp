...
while((opcion!=1)&&(opcion!=2));
    ...
    while(option!=4);
        ...
        float suma = 0;
        avl_tree<dni> test_tree;

            for(int i=0;i<npruebas;i++){

                randomizar(vector_prueba);

                for(unsigned int j=0;j<vector_prueba.size()-1;j++){
                    test_tree.insert(vector_prueba[j]);}

                test_tree.search(vector_prueba[size_tree-1]);
                comparacion[i] = test_tree.get_comparison();
                suma+=comparacion[i];
            ...
            vector<float> comparacion_2(vector_prueba_2.size()/2);
                ...
                for(int i=0; i<npruebas; i++){
                    ...
                    for(unsigned int j=0;j<(vector_prueba_2.size()/2)-1;j++){
